...
public class MouseManager : NetworkBehaviour
{
	...
	void Update()
	{
		...
		if (this.matchDataStr != matchDataBroadcaster.MatchDataStr)
		{
			this.matchDataStr = matchDataBroadcaster.MatchDataStr;
			this.matchData = JsonUtility.FromJson<MatchData>(this.matchDataStr);
		}
	...
	private void PlayingPhasePrimaryClick(string type, int x, int y)
	{
		...
		switch (type)
		{
			...
			case "MarketCard":
				...
				Debug.Log(debugTag + "Trying to find " + gridController.CreateCardObjectName("GameCard", this.ownerId, selection));
				oldSelection = GameObject.Find(gridController.CreateCardObjectName("GameCard", this.ownerId, selection));
